...
// Fetch posts assigned to the logged-in user and have the status 'Assigned'.
$assignedPostsStmt = $conn->prepare("SELECT Post_Id, PetName FROM petinfo WHERE Taken_by = ? AND Status = 'Assigned'");
$assignedPostsStmt->bind_param("s", $username);
$assignedPostsStmt->execute();
$assignedUserPosts = $assignedPostsStmt->get_result()->fetch_all(MYSQLI_ASSOC);
$assignedPostsStmt->close();
...
<div id="Feedback">
    ...
    <select name="post_id" id="post">
        <?php foreach ($assignedUserPosts as $post): ?>
...
document.addEventListener('DOMContentLoaded', function() {
    // You need to target the correct form if there are multiple forms on the page.
